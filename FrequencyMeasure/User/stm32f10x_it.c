...
void TIM6_IRQHandler(void){
	...
	typedef enum{ETR = 0,CAPTURE = 1} State;
	static State ETR_state = ETR,Last_State = ETR;
	if(TIM_GetFlagStatus(TIM6,TIM_FLAG_Update) == SET){
		...
		if(UnknownSig_fre_ETR > 100000){
			ETR_state = ETR;
			UnknownSig_fre = UnknownSig_fre_ETR;
			if(Last_State == ETR_state)
				TIM_ITConfig(TIM2,TIM_IT_Update | TIM_IT_CC3 | TIM_IT_CC4,DISABLE);//频率过大，失能Capture中断，防止卡死;
		}
		else if(UnknownSig_fre_ETR < 60000){
			ETR_state = CAPTURE;
